...
from data_utils.openx.action_tokenizer import ActionTokenizer
...
class OpenXMagma(Constructor):
    ...
    def __call__(self, item, video_path, visual_traces, width=512, height=512):
        ...
        # conversation 1: Q: what is the robot doing? A: task description        
        conversations = []
        ...
        # conversation 2: Q: to do the task, what should be the next action? A: next action
        conversations.extend([
            {"from": "human", "value": f"What action should the robot take to {gpt_response}?"},
            ...
            if len(mark_trace_future) > 0:
                ...
                if self.mm_use_trace_speed:
                    ...
                    conv_user = (
                        ...
                        f"The robot is doing: {gpt_response_todo}. To finish the task, how to move the numerical marks in the image with speed {avg_speed} for the next {step_to_predict} steps?\n"
                    )
                else:
                    conv_user = (
                        f"The image is split into {self.spatial_quant_size}x{self.spatial_quant_size} grids, and labeled with numeric marks: {mark_ids}.\n"
                        f"The robot is doing: {gpt_response_todo}. To finish the task, how to move the numerical marks in the image for the next {step_to_predict} steps?\n"
                    )
