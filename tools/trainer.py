...
class MagmaTrainer(Trainer):
    ...
    #     if self.loss_moving_average is not None:
    #         is_outlier = False
    #         # determine whether current loss is outlier
    #         if loss.item() > 1.3 * self.loss_moving_average:
    #             print(f"Outlier loss detected: {loss.item()} > {1.3 * self.loss_moving_average}")
    #             # turn down the loss
    #             loss = self.loss_moving_average + loss * 0.001
    #             is_outlier = True
    #         if not is_outlier:
    #             self.loss_moving_average = self.loss_moving_average * 0.99 + loss.item() * 0.01
    #         is_outlier = False
        
    #     if self.loss_moving_average is None:
    #         self.loss_moving_average = loss.item()
